...
if __name__ == "__main__":
    ...
    args = get_args()
    # this generates all possible binary strings of length n, by taking the
    # cartesian product {0..base}^n
    for code in product(range(args.base), repeat=args.n):
        # if we also want to show the data bits, print them to the screen
        if args.show_origin:
            print("".join(map(str, code)), end=",")
        # print the encoded data to the screen
        print("".join(map(str, hamming_encode(code, args.base))))
